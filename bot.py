...
С инлайн-формой для поиска по неполным данным.
"""
...
def build_form_keyboard(context):
    ...
    # Нижние кнопки
    keyboard.append([
        InlineKeyboardButton("🇷🇺 Россия", callback_data="set_russia"),
...
1107449004464 – ОГРН или ОГРНИП
    ...
    # Обработка кнопок редактирования полей (они вызовут переход в состояние EDITING)
        ...
        await query.edit_message_text(
            f"Введите новое значение для поля «{field}» (или отправьте /cancel для отмены):",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Отмена", callback_data="cancel_edit")]])
        )
        return EDITING

    elif data == "set_russia":
        # Устанавливаем место рождения = "Россия"
        if 'search_form' not in context.user_data:
            context.user_data['search_form'] = {}
        context.user_data['search_form']['birth_place'] = "Россия"
        text = build_form_message(context)
        keyboard = build_form_keyboard(context)
        await query.edit_message_text(text, reply_markup=keyboard, parse_mode='Markdown')
        return
        ...
        # Формируем запрос из заполненных полей
        data = context.user_data.get('search_form', {})
        parts = []
...
async def handle_edit_input(update, context):
    ...
    if not text:
        await update.message.reply_text("Пустое значение не допускается. Попробуйте снова или /cancel.")
        return EDITING
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^(search_form|examples|profile|partner|buy|main|cancel_search|set_russia|reset_form|do_search|edit_.*)$"))

    # Диалог для редактирования полей формы
    conv_handler = ConversationHandler(
        ...
        fallbacks=[
            ...
            CallbackQueryHandler(menu_callback, pattern="^cancel_edit$")
        ],
        allow_reentry=True
    )
    ...
    # Обычные сообщения (быстрый поиск)
