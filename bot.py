...
Инлайн-форма для поиска по неполным данным с SEON и Snusbase.
"""
...
def format_result(results, query, search_type):
    ...
    def add_source_data(source_name, data, max_items=10):
        ...
        if isinstance(data, list) and len(data) > 0:
            ...
            # Для SEON и Snusbase показываем ключи
                ...
                if k not in ['error', 'status'] and v:
                    ...
                    if isinstance(v, dict):
                        for sub_k, sub_v in v.items():
                            if sub_v:
                                lines.append(f"  {get_field_name(sub_k)}: {sub_v}")
                    else:
                        lines.append(f"  {get_field_name(k)}: {v}")
        else:
            lines.append(f"{source_name}: {data}")

    for source in ['depsearch', 'jitler', 'leak', 'funstat', 'dadata', 'seon', 'snusbase']:
...
"""
    ...
    if seon and isinstance(seon, dict) and "error" not in seon:
        html += '<div class="section"><h2>SEON</h2><pre>' + json.dumps(seon, indent=2, ensure_ascii=False) + '</pre></div>'

    snusbase = results.get("snusbase")
    if snusbase and isinstance(snusbase, dict) and "error" not in snusbase:
        html += '<div class="section"><h2>Snusbase</h2><pre>' + json.dumps(snusbase, indent=2, ensure_ascii=False) + '</pre></div>'
