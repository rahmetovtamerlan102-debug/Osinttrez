...
Автоопределение типа, без смайликов, HTML-отчёт, Flask для UptimeRobot
"""
...
def unified_search(query, search_type):
    ...
    if search_type == "phone":
        ...
        clean = clean_phone(query) if query.isdigit() else query
...
def format_result(results, query, search_type):
    ...
    if search_type == "phone":
        ...
        if seon:
            ...
            if seon.get("score") is not None:
                lines.append(f"Риск: {seon.get('score')}")
            if seon.get("email"):
                lines.append(f"Email: {seon.get('email')}")
    elif search_type == "telegram_id":
        lines.append(f"ID Telegram: {clean_phone(query) if query.isdigit() else query}")
    elif search_type == "email":
    ...
    if results.get("seon"):
        sources += 1
    if results.get("snusbase") and isinstance(results["snusbase"], dict) and "error" not in results["snusbase"]:
        sources += 1
