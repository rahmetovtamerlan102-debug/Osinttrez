...
Инлайн-форма для поиска по неполным данным с красивым дизайном.
"""
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if data.get("result"):
                    ...
                    if "id" in data:
                        for _ in range(5):  # увеличил попытки
                            time.sleep(1.5)
...
# ========== ФОРМАТИРОВАНИЕ КРАТКОГО ОТВЕТА (улучшенное) ==========
def format_result(results, query, search_type):
    ...
    def add_source_data(source_name, data, max_items=10):
        nonlocal lines
        if not data:
            return
        if isinstance(data, dict) and "error" in data:
            lines.append(f"❌ *{source_name}*: {data['error']}")
            return
        if isinstance(data, list) and len(data) > 0:
            lines.append(f"📂 *{source_name}* (найдено {len(data)} записей):")
            for idx, item in enumerate(data[:max_items], 1):
                if isinstance(item, dict):
                    # Формируем строку из полей
                    parts = []
                    for k, v in item.items():
                        if v and k not in ['id', 'source']:
                            parts.append(f"{get_field_name(k)}: {v}")
                    if parts:
            ...
            if len(data) > max_items:
                lines.append(f"  ... и ещё {len(data)-max_items} записей")
        elif isinstance(data, dict):
            # Если словарь, выводим ключи-значения
            lines.append(f"📂 *{source_name}*:")
            for k, v in data.items():
                if k not in ['error', 'status'] and v:
                    lines.append(f"  {get_field_name(k)}: {v}")
        else:
            lines.append(f"📂 *{source_name}*: {data}")

    for source in ['depsearch', 'jitler', 'leak', 'funstat', 'dadata']:
        data = results.get(source)
        if data:
            add_source_data(source.upper(), data)
...
# ========== ФУНКЦИЯ ПОСТРОЕНИЯ ФОРМЫ (убрана лишняя фраза) ==========
def build_form_message(context):
    ...
    for label, key in fields:
        ...
        if not value:
            value = '—'
        lines.append(f"*{label}:* {value}")  # жирный шрифт
    text = intro + "\n".join(lines)  # без дополнительной фразы
    return text
            ...
            try:
                ...
                if referrer_id == user_id:
                    ...
                    try:
                        ...
                        referrer_username = referrer_user.username or str(referrer_id)
                    except:
...
1107449004464 – ОГРН или ОГРНИП
        """
        await query.edit_message_text(examples_text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Назад", callback_data="main")]]), parse_mode='Markdown')
        ...
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Назад", callback_data="main")]]), parse_mode='Markdown')
        ...
        await msg.edit_text(formatted, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
        return ConversationHandler.END
...
async def repeat_callback(update, context):
    ...
    await query.edit_message_text(formatted, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
