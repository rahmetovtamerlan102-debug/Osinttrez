...
Быстрый, без голубых кнопок, автоопределение, HTML-отчёт
"""
...
TIMEOUT = int(os.getenv("TIMEOUT", "15"))  # быстрые таймауты
...
def jitler_wait_result(task_id, token, attempts=8):
...
def leakosint_search(query, limit=200, retries=2):
    ...
    for _ in range(retries):
        try:
            resp = requests.post(LEAKOSINT_BASE, json=data, timeout=20)
            if resp.status_code == 200:
                result = resp.json()
...
def detect_search_type(text):
    ...
    if text.startswith('@') or (not text.isdigit() and re.search(r'[a-zA-Z]', text)):
        return "telegram_id"  # username или логин
...
def unified_search(query, search_type):
    ...
    if search_type == "phone":
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            results["dadata"] = dadata_future.result(timeout=8) if dadata_future else None
            results["leak"] = leak_future.result(timeout=20) if leak_future else {"error": "Timeout"}
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            results["jitler"] = jitler_future.result(timeout=TIMEOUT) if jitler_future else {"error": "Timeout"}
            results["depsearch"] = dep_future.result(timeout=TIMEOUT) if dep_future else {"error": "Timeout"}
            results["funstat"] = funstat_future.result(timeout=12) if funstat_future else None
            results["leak"] = leak_future.result(timeout=20) if leak_future else {"error": "Timeout"}
            results["dadata"] = None
...
async def require_subscription(update, context):
    keyboard = [
        [KeyboardButton("Подписаться", url=f"https://t.me/{REQUIRED_CHANNEL[1:]}")],
...
async def check_subscription(update, context):
    ...
    if await is_subscribed(user_id, context):
        await update.message.reply_text("Подписка подтверждена!", reply_markup=main_menu_keyboard())
    else:
        await update.message.reply_text(
            "Вы ещё не подписаны. Подпишитесь и нажмите «Проверить подписку» снова.",
            reply_markup=ReplyKeyboardMarkup([["Подписаться", "Проверить подписку"]], resize_keyboard=True)
        )
...
async def handle_menu(update, context):
    ...
    elif text == "Примеры использования сервиса":
        examples = """
...
1107449004464 – ОГРН или ОГРНИП
        """
        await update.message.reply_text(examples, reply_markup=main_menu_keyboard())
        ...
        # Если текст не совпадает с кнопками – ищем
...
async def repeat_callback(update, context):
    ...
    if not results or not query_text:
        await query.edit_message_text("Нет данных для повтора. Начните новый поиск.", reply_markup=main_menu_keyboard())
        return
...
def main():
    ...
    app.add_handler(MessageHandler(filters.Regex("Проверить подписку"), check_subscription))
    app.add_handler(MessageHandler(filters.Regex("^(Поиск|Примеры|Мой|Партнёрская|Назад)$"), handle_menu))
