#!/usr/bin/env python3
"""
InfoHunt Telegram Bot – быстрый поиск.
"""
...
SEON_URL = "https://api.seon.io/SeonRestService/phone-api/v1"
...
def create_user(user_id, referrer_id=None):
    ...
    if referrer_id:
        ...
        conn.commit()
        conn.close()
        # Начисляем реферальный баланс (дополнительно)
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
...
def reset_daily_queries_if_needed(user_id):
    ...
    if user['last_reset_date'] != today:
        ...
        if reg_date == today:
            ...
            new_queries = 2  # ежедневно 2 запроса
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
...
def get_working_jitler_token():
    for token in JITLER_TOKENS:
        try:
            resp = requests.get("https://api.jitler.top/me", headers={"Authorization": f"Bearer {token}"}, timeout=3)
            if resp.status_code == 200 and resp.json().get("result", False):
                return token
        except:
            continue
...
def jitler_search(query, search_type="number"):
    ...
    jitler_type = "number" if search_type in ("number", "phone") else "sherlock"
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if data.get("result") and "id" in data:
                for _ in range(4):
                    time.sleep(1)
...
def leakosint_search(query, limit=100):
    ...
    token = None
    for t in LEAKOSINT_TOKENS:
        try:
            data = {"token": t, "request": "test", "limit": 5, "lang": "ru"}
            r = requests.post(LEAKOSINT_BASE, json=data, timeout=3)
            if r.status_code == 200 and "Error code" not in r.json():
                token = t
                break
        except:
            continue
...
def seon_lookup(phone):
    clean = re.sub(r'\D', '', str(phone))
    if not clean or len(clean) < 10 or not SEON_TOKEN:
        return None
    try:
        resp = requests.post(
            SEON_URL,
        ...
        if resp.status_code == 200:
            return resp.json()
        return {"error": f"SEON {resp.status_code}"}
    except:
        return {"error": "SEON не ответил"}

def snusbase_search(query):
    if not query or not SNUSBASE_TOKEN:
        return None
    try:
        resp = requests.post(
            SNUSBASE_URL,
            json={"terms": [query.strip()], "types": ["email"], "wildcard": False},
            headers={"Auth": SNUSBASE_TOKEN, "Content-Type": "application/json"},
            timeout=10
        )
        if resp.status_code == 200:
            return resp.json()
        return {"error": f"Snusbase {resp.status_code}"}
    except:
...
def detect_search_type(text):
    ...
    if re.fullmatch(r'\d+', text):
        return "telegram_id" if len(text) > 15 else "phone"
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for key, future in futures.items():
            try:
                ...
                results[key] = {"error": "Timeout или ошибка"}

        for k in ["jitler", "depsearch", "dadata", "leak", "seon", "snusbase", "funstat"]:
            if k not in results:
                results[k] = None
    return results
