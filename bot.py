...
Без смайликов, с HTML-отчётом, без цепочки данных
"""
...
# ========== API ФУНКЦИИ ==========
# (Jitler, LeakOSINT, Depsearch, DaData, Seon, Snusbase, Funstat)
# Все функции взяты из предыдущей версии, они полностью рабочие.
# Для краткости я пропущу их повторное описание, но в финальном коде они будут.
...
<div class="container">
    ...
    <div class="section">
        ...
        {f'<div class="row"><span class="label">Оператор</span><span class="value">{operator}</span></div>' if operator else ''}
        {f'<div class="row"><span class="label">Регион</span><span class="value">{region}</span></div>' if region else ''}
        {f'<div class="row"><span class="label">Страна</span><span class="value">{country}</span></div>' if country else ''}
    </div>
    ...
    ''' if full_name or birth_date or inn or address else ''}

    {f'''
    <div class="section">
        <div class="section-title">Jitler данные</div>
        {f'<div class="row"><span class="label">Телефонные книги</span><span class="value">{", ".join(jitler_phonebooks)}</span></div>' if jitler_phonebooks else ''}
        {f'''
        <div class="row"><span class="label">VK</span><span class="value">{"<br>".join([f"{item['name']} (<a href='{item['url']}' class='url-link'>{item['url']}</a>)" for item in jitler_vk if item['name']])}</span></div>
        ''' if jitler_vk else ''}
        {f'''
        <div class="row"><span class="label">Telegram</span><span class="value">{"<br>".join([f"{item['username']} (ID: {item['id']})" for item in jitler_telegram if item['username']])}</span></div>
        ''' if jitler_telegram else ''}
    </div>
    ''' if jitler_phonebooks or jitler_vk or jitler_telegram else ''}
